...
GLuint loadShader(const char *vertexfilename, const char *fragmentfilename) {
  ...
  std::string vertexString = getTextFile(vertexfilename);
  std::string fragmentString = getTextFile(fragmentfilename);
  const char * vertexFile = vertexString.c_str();
  const char * fragmentFile = fragmentString.c_str();
  // glCreateShader
  vertex_shader_id = glCreateShader(GL_VERTEX_SHADER);
  // glShaderSource
  glShaderSource(vertex_shader_id, 1, &vertexFile, NULL);
  // glCompileShader
  glCompileShader(vertex_shader_id);
  // glCreateShadeß
  fragment_shader_id = glCreateShader(GL_FRAGMENT_SHADER);
  // glShaderSource
  glShaderSource(fragment_shader_id, 1, &fragmentFile, NULL);
  // glCompileShader
